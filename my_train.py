...
def main():
    ...
    parser.add_argument('-c', '--config', default='config.yaml', help='baseline config file')
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename="parcnet-is2_baseline_{epoch:03d}",
    ...
    cht
     checkpoint_callback = ModelCheckpoint(
        ...
        dirpath=tb_logger.log_dir + '/checkpoints',
        monitor='train_loss',  # Monitora la perdita durante il training
        filename="parcnet-is2_baseline_{epoch:03d}_{train_loss:.2f}",
        save_on_train_epoch_end=True,
        save_weights_only=True,
        save_top_k=1,  # Salva solo il miglior modello in base alla loss
        save_last=True,  # Salva sempre l'ultimo modello, indipendentemente dal monitor
        verbose=True,
    ...
    datamodule = NMPDataModule(
        audio_dir=config['path']['source_dataset_dir'],
